...
public class UniqueAttributeValidatorProvider extends AbstractStringValidator implements ConfiguredProvider {
    ...
    public static final String MESSAGE_ATTRIBUTE_PHONE_NOT_UNIQUE = "User already exists with this phone number";
    ...
    @Override
    protected void doValidate(String attributeValue, String attributeName, ValidationContext context,
        ...
        if (!isAttributeUnique(attributeValue, attributeName, session, currentUser)) {
            if (attributeName.equals("phoneNumber")) {
                context.addError(new ValidationError(ID, attributeName,
                        MESSAGE_ATTRIBUTE_PHONE_NOT_UNIQUE));
            } else {
                context.addError(
                        new ValidationError(ID, attributeName, MESSAGE_ATTRIBUTE_NOT_UNIQUE + '.' + attributeName));
            }
        }
    }

    public boolean isAttributeUnique(String attributeValue, String attributeName, KeycloakSession session,
            UserModel currentUser) {
        UserProvider userProvider = session.getProvider(UserProvider.class);

        RealmModel realm = this.getRealm(session);

        Stream<UserModel> usersWithSameAttributeValue = userProvider.searchForUserByUserAttributeStream(realm,
                attributeName, attributeValue);

        return currentUser != null
                ? usersWithSameAttributeValue.filter(user -> !user.getId().equals(currentUser.getId())).findAny()
                        .isEmpty()
    ...
    public String getHelpText() {
        return "Unique Attribute as phone number or etc...";
    }
